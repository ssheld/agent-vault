...
Always-on memory is paid on every session and re-paid after every compaction, so keep it lean; treat budget overflow as a defect rather than cosmetic.
- Three budget buckets are reported by the agent-vault `check-memory-budget.sh` tool: the Claude/Gemini `@`-import chain (auto-discovered from `CLAUDE.md`/`GEMINI.md`; loaded every session), the Codex `AGENTS.md` chain (concatenated up to Codex `project_doc_max_bytes`), and protocol-read files (named by session start; read, not auto-imported). Defaults: per-file budget 40000 chars, `@`-chain budget 120000 chars.
...
- Preserve active invariants. Never archive current runtime behavior as if it were closed-issue history. A narrative file may stay above its per-file budget as a documented exception when shrinking it further would delete a live invariant; record the file and reason in the budget tool's `--exceptions` file so the report prints why.
- Roll over `context-log.md` instead of letting it grow: keep exactly one `## Current Snapshot` plus a recent-entries window, move older entries into `agent-vault/context/archive/context-log-YYYY.md`, label any archived snapshot superseded, and validate with the agent-vault `check-context-log-rollover.sh` tool.
