...
Always-on memory is paid on every session and re-paid after every compaction, so keep it lean; treat budget overflow as a defect rather than cosmetic. The `check-memory-budget.sh` and `check-context-log-rollover.sh` checkers named below ship with the agent-vault repository; until a project installs them, follow these conventions as the contract and run the checkers from an agent-vault checkout.
- Three budget buckets (reported by `check-memory-budget.sh` when available): the Claude/Gemini `@`-import chain (auto-discovered from `CLAUDE.md`/`GEMINI.md`; loaded every session), the Codex `AGENTS.md` chain (all `AGENTS.md` files, concatenated up to Codex `project_doc_max_bytes`), and protocol-read files (named by session start; read, not auto-imported). Defaults: per-file budget 40000 chars (the Claude Code memory-file warning threshold), `@`-chain budget 120000 chars; a repo can override budgets and file sets in a committed `agent-vault/memory-budget.config`.
...
- Preserve active invariants. Never archive current runtime behavior as if it were closed-issue history. A narrative file may stay above its per-file budget as a documented exception when shrinking it further would delete a live invariant; record the file and reason in an exceptions file (or `agent-vault/memory-budget.config`) so the budget report prints why.
- Roll over `context-log.md` instead of letting it grow: keep exactly one `## Current Snapshot` plus a recent-entries window, move older entries into `agent-vault/context/archive/context-log-YYYY.md`, label any archived snapshot superseded, and validate with `check-context-log-rollover.sh` when available.
- Classify each archived `lessons.md` entry as retained-as-quick-rule, covered-by-a-named-always-on-rule (name it), or archival-only with low recurrence risk.
