...
- Both helpers refuse automatic stale-record pruning if another registered worktree is missing or marked prunable. Git pruning is repository-wide; investigate and restore/repair other records, or deliberately preview and inspect all proposed removals before pruning.
...
Memory helpers require Bash 4.4+; generated hooks remain Bash 3.2-compatible. An unsupported optional checker reports a non-blocking advisory, not successful validation. See `agent-vault/_assets/hooks/README.md` for setup and managed-refresh guidance.

The import report measures selected-root, repo-local unique source bytes, not exact expanded client context. Known external imports are listed exclusions; incomplete in-scope analysis fails `--strict`. Managed-helper refreshes may therefore affect custom strict CI, but the generated pre-commit report stays non-blocking. These checker profiles do not change the session-start load instructions.
Always-on memory is paid on every session and re-paid after every compaction, so keep it lean; treat budget overflow as a defect rather than cosmetic. The `scripts/check-memory-budget.sh` and `scripts/check-context-log-rollover.sh` checkers are installed in each project's `scripts/` (seeded by `new-project.sh`, refreshed by `update-project.sh`); run them to apply the conventions below. When the corresponding checker is executable, the pre-commit hook reports two independent, non-blocking warnings against staged content: memory-budget findings when memory/instruction paths are staged, and structural rollover findings when `agent-vault/context-log.md` has a staged blob (a staged deletion has none). The budget check measures the staged index; the rollover check reads only that context-log blob. Both run even with `AGENT_VAULT_SKIP_METADATA_GATE=1`. Silence only the budget warning with `AGENT_VAULT_SKIP_MEMORY_BUDGET=1`, or only the rollover warning with `AGENT_VAULT_SKIP_ROLLOVER_CHECK=1`; neither bypasses metadata enforcement or triggers automatic compaction. With partial staging, inspect `git diff --cached` or `git show :agent-vault/context-log.md` before relying on a working-tree check. See `agent-vault/_assets/hooks/README.md` for the warning triggers and commands.
- Budget buckets (reported by `scripts/check-memory-budget.sh`): the Claude `@`-import chain (from `CLAUDE.md`) and the Gemini `@`-import chain (from `GEMINI.md`), budgeted separately because a session loads one chain not the union; the Codex `AGENTS.md` chain (all `AGENTS.md` files, informational against Codex `project_doc_max_bytes`); and protocol-read files (named by session start; read, not auto-imported). Sizes are bytes. Defaults: general per-file 40000 bytes, per `@`-chain 120000 bytes. The designated context log alone has a 60000-byte allowance in the protocol-read bucket; importing it still uses the general limit and existing chain/exception accounting. Configure `context_log_budget`, `context_log_target` (default 30000), and `context_log_path` (default `agent-vault/context-log.md`) in `agent-vault/memory-budget.config`; override the path with `--context-log-path`. An explicit designation must be in effective `protocol_read` (otherwise exit 2); excluding the built-in default alone is informational. Use explicit `compact-context-log.sh --to-budget` to retain whole entries within the target after crossing the trigger; a target-infeasible rollover refuses without writes unless `--allow-target-overage` is explicitly selected. `--keep N` keeps its count-based behavior and ignores budget config; recovery also ignores it. No hook runs compaction automatically. Both context values must be positive decimal integers with target < budget. Designation never adds a file to the protocol set.
...
- Roll over `context-log.md` instead of letting it grow: keep exactly one `## Current Snapshot` plus a recent-entries window, move older entries into `agent-vault/context/archive/context-log-YYYY.md`, label any archived snapshot superseded, and validate with `scripts/check-context-log-rollover.sh`. Prefer `scripts/compact-context-log.sh`: it validates and stages all outputs before replacement, then records enough state for explicit roll-forward recovery. Individual renames are atomic; the three-file rollover is not. Use a single writer for the entire log/archive/manifest set, including recovery. On exit 3, stop the original writer and follow the diagnostic: recover a recorded transaction with `scripts/compact-context-log.sh <log> --recover` (`--dry-run` previews recovery); do not retry a fresh rollover or clean/move `.agent-vault-rollover-*` directories while pending. Missing/corrupt records or diverged outputs may require manual reconciliation; a missing-record safety check refuses recognizable live/archive entry overlap and inconsistent manifest/live pointers. For a verified manual rollover pointer with no manifest records, `--dry-run` can preview adoption and the one-time `--adopt-manual-rollover` option initializes the first manifest on the next write-producing rollover; recovery, overlap, and session gates still apply. Gate/validation failures leave outputs unchanged. The compactor fails closed on an archive that carries its own frontmatter `covers:` field or relocation manifest — it cannot keep that header in sync, so roll those archives over manually, or pass `--allow-stale-archive-metadata` and update the header by hand. When rolling over manually, finalize the durable descriptions (archive boundary, kept/archived counts) *after* the gate-required session entry is in place; name the boundary by topic or recent-window position rather than a bare timestamp (two same-day entries can share a minute); and cite the stable rolled size with a `~` rather than an exact live byte total (it drifts on the next edit). Keep the live pointer's boundary and the archive's own frontmatter/manifest pointing at the same actual newest archived entry.
